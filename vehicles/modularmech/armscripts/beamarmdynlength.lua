...
require("/scripts/xcore/LoggingOverride.lua") -- tl;dr I can use print, warn, error, assert, and assertwarn
require("/vehicles/modularmech/armscripts/base.lua")

-- Written by Chucklefish
-- Modified by Sayter
-- Modified AGAIN by Xan the Dragon // Eti the Spirit
...
function BeamArm:fireState()
	...
	if beamCollision and self.beamTileDamage > 0 then
		self.maximumEndPoint = vec2.add(self.firePosition, vec2.mul(self.aimVector, beamLength)) -- don't use self.beamLength
...
function BeamArm:updateBeam()
	local endPoint;
	if self.endAtCursor then
		endPoint = self.aimPosition
	else
		endPoint = vec2.add(self.firePosition, vec2.mul(self.aimVector, self.beamLength))
	end
	local beamCollision = world.lineCollision(self.firePosition, endPoint)
	if beamCollision then
		endPoint = beamCollision
	end
	local beamLength = math.min(world.magnitude(self.firePosition, endPoint), self.maxBeamLength or 30)
